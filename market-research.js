...
{
	//If we dont use any format, we get:
	//    http://finance.yahoo.com/d/quotes.csv?s=%s&f=%sEQM+Vsnj1pr
	//If we get some format, we get:
	//    http://finance.yahoo.com/d/quotes.csv?s=EQM+V&f=snj1pr
    return util.format(
...
{
	// we multiply by 1e9 to transform as an example:
	// 292.1B877.07 -> 292100000000
...
var csv2console = function(csvfile, headers)
{
	...
	csv().from.path(csvfile).on('record', function(row, index)
	{
		var shares = Math.round(marketCapFloat(row[2])/row[3], 0);
		// we only want 3 decimal number
...
if (require.main == module)
{
	...
	if ( symbols.length > 2)
	{
		// Symbol has this content: 
		//[ 'node',
        //  'absolute_path/prova.js',
        //  'EQM',
        //  'V' ]
        // so we get tickers from pos 2
